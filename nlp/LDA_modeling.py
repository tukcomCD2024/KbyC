...
# LDA 모델 학습 및 결과 저장
def train_lda_model_and_save_topics(documents, num_topics=5, word_filter_file='stopword.txt'):
    # 문서 단어 사전 생성
    ...
    # topics.csv 파일로 저장
    df_topics.to_csv('./outputs/topics.csv', encoding='utf-8-sig', index=False)
    ...
    # result.csv 파일로 저장
    result.to_csv('./outputs/result.csv', encoding='utf-8-sig', index=False)
